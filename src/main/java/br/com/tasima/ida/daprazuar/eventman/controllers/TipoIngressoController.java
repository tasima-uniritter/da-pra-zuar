...
public class TipoIngressoController {
    ...
    public ResponseEntity<TipoIngresso> Create(@PathVariable long eventoId, @RequestBody TipoIngresso ti) throws DateException {
    	try {
    		Evento ev = eventoService.get(eventoId);
	...
	public ResponseError customError(HttpServletRequest req, Exception ex) {
		ResponseError response = new ResponseError(1, ex.getMessage());
		return response;
	}
}
